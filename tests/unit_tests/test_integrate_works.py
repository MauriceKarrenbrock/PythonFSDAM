...
class Testget_work_profile_vs_lambda():
    def test_zeros(self, mocker):
        lambda_work = []
        lambda_work.append([0., 0.5, 1.])
        lambda_work.append([0., 0., 0.])

        m_integrator = mocker.patch(
            'PythonFSDAM.integrate_works.integrate_work_profiles')
        m_empty = mocker.patch('numpy.empty')

        m_empty.return_value = [0.] * 20

        wrk.get_work_profile_vs_lambda(lambda_work)

        m_integrator.assert_called()
        m_empty.assert_called_once()
